...
function updateStatsSummary(stats) {
  ...
  if (titleSection) {
    ...
    summaryDiv.className = 'grid grid-cols-2 gap-3 mb-8';
    summaryDiv.innerHTML = `
      <div class="bg-gradient-to-br from-ink to-ink-light text-paper p-4 rounded-xl">
        <p class="text-paper/60 text-xs mb-1">总字数</p>
        <p class="font-calligraphy text-3xl">${stats.totalCharacters.toLocaleString()}</p>
      </div>
      <div class="bg-gradient-to-br from-cinnabar to-red-400 text-white p-4 rounded-xl">
        <p class="text-white/60 text-xs mb-1">不重复字</p>
        <p class="font-calligraphy text-3xl">${stats.uniqueCharacters.toLocaleString()}</p>
      </div>
      <div class="bg-gradient-to-br from-jade to-emerald-600 text-white p-4 rounded-xl">
        <p class="text-white/60 text-xs mb-1">道经/德经</p>
        <p class="font-calligraphy text-3xl">${stats.daoJingStats.chapters}/${stats.deJingStats.chapters}</p>
      </div>
      <div class="bg-gradient-to-br from-gold to-amber-500 text-white p-4 rounded-xl">
        <p class="text-white/60 text-xs mb-1">平均每章</p>
        <p class="font-calligraphy text-3xl">${stats.avgCharsPerChapter}字</p>
      </div>
    `;
    ...
    <div class="space-y-3">
      ${daoData.themes.slice(0, 8).map((theme, index) => `
        <div class="flex items-center gap-3">
          <span class="text-sm w-24 truncate" title="${theme.description}">${theme.name}</span>
          <div class="flex-1 h-6 bg-paper rounded-full overflow-hidden relative">
            <div class="h-full rounded-full transition-all duration-1000 ${
              index % 4 === 0 ? 'bg-cinnabar' : index % 4 === 1 ? 'bg-jade' : index % 4 === 2 ? 'bg-gold' : 'bg-blue-500'
            }" style="width: ${(theme.count / maxThemeCount * 100)}%"></div>
          </div>
          <span class="text-xs text-ink-light w-8">${theme.count}</span>
        </div>
      `).join('')}
    </div>
  `;
...
function initChapterLengthChart() {
  ...
  lengthChart.innerHTML = `
    <div class="flex flex-wrap gap-[2px] justify-center mb-4">
      ${lengths.map((item, idx) => `
        <div class="w-2 rounded-t-sm ${
          item.type === '道经' ? 'bg-cinnabar' : 'bg-jade'
        } hover:opacity-80 cursor-pointer transition-all"
             style="height: ${Math.max(4, item.len / maxLength * 80)}px;"
             title="第${item.id}章: ${item.len}字" onclick="showChapterDetail(${item.id})"></div>
      `).join('')}
    </div>
    <div class="flex justify-center gap-4 text-xs">
      <span class="flex items-center gap-1"><span class="w-3 h-3 bg-cinnabar rounded"></span>道经(1-37)</span>
      <span class="flex items-center gap-1"><span class="w-3 h-3 bg-jade rounded"></span>德经(38-81)</span>
    </div>
    <p class="text-xs text-center text-ink-light mt-2">点击柱状图查看章节详情</p>
  `;
}
    ...
    <div class="bg-white p-4 rounded-xl shadow-sm">
      <div class="grid grid-cols-2 gap-4 mb-4">
        <div class="text-center p-3 bg-red-50 rounded-lg">
          <p class="text-xs text-cinnabar mb-1">道经 (第1-37章)</p>
          <p class="font-calligraphy text-2xl">${stats.daoJingStats.chapters}章</p>
          <p class="text-xs text-ink-light">${stats.daoJingStats.totalChars}字 · 平均${stats.daoJingStats.avgChars}字/章</p>
        </div>
        <div class="text-center p-3 bg-green-50 rounded-lg">
          <p class="text-xs text-jade mb-1">德经 (第38-81章)</p>
          <p class="font-calligraphy text-2xl">${stats.deJingStats.chapters}章</p>
          <p class="text-xs text-ink-light">${stats.deJingStats.totalChars}字 · 平均${stats.deJingStats.avgChars}字/章</p>
        </div>
      </div>
      <div class="h-4 bg-gray-100 rounded-full overflow-hidden flex">
        <div class="h-full bg-cinnabar transition-all duration-1000" style="width: ${stats.daoJingStats.totalChars / stats.totalCharacters * 100}%"></div>
        <div class="h-full bg-jade transition-all duration-1000" style="width: ${stats.deJingStats.totalChars / stats.totalCharacters * 100}%"></div>
      </div>
      <p class="text-xs text-center text-ink-light mt-2">
        字数占比: 道经${(stats.daoJingStats.totalChars / stats.totalCharacters * 100).toFixed(1)}% : 德经${(stats.deJingStats.totalChars / stats.totalCharacters * 100).toFixed(1)}%
      </p>
    </div>
  `;
        ...
        ${stats.keyConcepts.map(concept => `
          <div>
            <div class="flex justify-between text-sm mb-1">
              <span class="font-semibold">${concept.concept}</span>
              <span class="text-xs text-ink-light">比例 ${concept.ratio}</span>
            </div>
            <div class="h-6 bg-gray-100 rounded-full overflow-hidden flex text-xs">
              <div class="h-full bg-cinnabar flex items-center justify-center text-white"
                   style="width: ${concept.daojing / (concept.daojing + concept.dejing) * 100}%">${concept.daojing}</div>
              <div class="h-full bg-jade flex items-center justify-center text-white"
                   style="width: ${concept.dejing / (concept.daojing + concept.dejing) * 100}%">${concept.dejing}</div>
            </div>
          </div>
        `).join('')}
      </div>
      <p class="text-xs text-ink-light mt-4 text-center">红色=道经出现次数，绿色=德经出现次数</p>
    </div>
  `;
...
function initLengthDistribution(stats) {
  ...
  section.innerHTML = `
    <h3 class="font-serif text-lg mb-4 flex items-center">
      <span class="w-1 h-6 bg-teal-500 rounded-full mr-3"></span>
      章节长度分布
    </h3>
    <div class="bg-white p-4 rounded-xl shadow-sm">
      <div class="grid grid-cols-2 gap-3">
        <div class="p-3 bg-blue-50 rounded-lg text-center">
          <p class="text-xs text-blue-600 mb-1">短篇</p>
          <p class="font-calligraphy text-2xl text-blue-700">${dist.short.count}</p>
          <p class="text-xs text-ink-light">${dist.short.range} · ${dist.short.percentage}%</p>
        </div>
        <div class="p-3 bg-green-50 rounded-lg text-center">
          <p class="text-xs text-green-600 mb-1">中篇</p>
          <p class="font-calligraphy text-2xl text-green-700">${dist.medium.count}</p>
          <p class="text-xs text-ink-light">${dist.medium.range} · ${dist.medium.percentage}%</p>
        </div>
        <div class="p-3 bg-orange-50 rounded-lg text-center">
          <p class="text-xs text-orange-600 mb-1">长篇</p>
          <p class="font-calligraphy text-2xl text-orange-700">${dist.long.count}</p>
          <p class="text-xs text-ink-light">${dist.long.range} · ${dist.long.percentage}%</p>
        </div>
        <div class="p-3 bg-purple-50 rounded-lg text-center">
          <p class="text-xs text-purple-600 mb-1">超长篇</p>
          <p class="font-calligraphy text-2xl text-purple-700">${dist.extraLong.count}</p>
          <p class="text-xs text-ink-light">${dist.extraLong.range} · ${dist.extraLong.percentage}%</p>
        </div>
      </div>
      <div class="mt-4 p-3 bg-gray-50 rounded-lg">
        <p class="text-xs text-ink-light">
          <span class="font-semibold">最长:</span> 第${stats.longestChapter.id}章 · ${stats.longestChapter.chars}字 · ${stats.longestChapter.title}<br>
          <span class="font-semibold">最短:</span> 第${stats.shortestChapter.id}章 · ${stats.shortestChapter.chars}字 · ${stats.shortestChapter.title}
        </p>
      </div>
    </div>
  `;
